...
int t_export SaveRef(HWND aparent, LPImportRef Ref, LPMMIOINFO FS, _TCHAR *INIPath){
  ...
  switch (Ref->RefType){
    case 0: SaveField(FS, Ref->Journal, _T("%%J %s\r\n"));break;
    case 1: SaveField(FS, Ref->Journal, _T("%%B %s\r\n"));break;
    case 2: SaveField(FS, Ref->Journal, _T("%%R %s\r\n"));
  }
                                        ...
                                        _TCHAR *INIPath)
{
    ...
    {  // New field
	  ...
	  OemToAnsi(tstr1, tstr1);
        ...
        switch (tstr1[1])
        {
          ...
          case _T('C'): DOFIELD(Tmp.City);break;
          case _T('D'): DOFIELD(Tmp.Date);break;
          case _T('I'): DOFIELD(Tmp.Publisher);break;
          case _T('J'):
          case _T('B'):
          case _T('R'):{
            DOFIELD(Tmp.Journal);
            switch (tstr1[1]){
              case _T('J'): Tmp.RefType = 0; break;
              case _T('B'): Tmp.RefType = 1; break;
              case _T('R'): Tmp.RefType = 2;
            }
            break;
          }
          case _T('K'):{
          ...
          case _T('P'): DOFIELD(Tmp.Pages);break;
          case _T('T'): DOFIELD(Tmp.Title);break;
          case _T('V'): DOFIELD(Tmp.Vol);break;
          case _T('N'): DOFIELD(Tmp.VolNo);break;
          case _T('X'): DoAbstract(&tstr1[3], &Tmp); break;
          default: DoAbstract(tstr1, &Tmp);
        }
      }
    } else
  ...
  if (Tmp.Keywords){  // place each keyword on a separate line
...
void Opt_OnCommand(HWND Dialog, int id, HWND hwndCtl, UINT codeNotify){
  switch (id){
    case IDOK:EndDialog(Dialog, IsDlgButtonChecked(Dialog, 101)); break;
    case IDCANCEL: EndDialog(Dialog, -1); break;
  }
}
